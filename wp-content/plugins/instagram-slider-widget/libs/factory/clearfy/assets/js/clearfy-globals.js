 ...
 * и самого Clearfy.
 *
 * $.wbcr_factory_clearfy_234.app - методы для работы с приложением. Скрыть, показать уведомления.
 * $.wbcr_factory_clearfy_234.hooks - это иммитация хуков и фильтров аналогично тем, что используются в Wordpress
 *
...
(function($) {
	...
	if( !$.wbcr_factory_clearfy_234 ) {
		$.wbcr_factory_clearfy_234 = {};
	}

	//todo: Переопредление для совместимости со старыми версиями плагинов.
	$.wbcr_factory_clearfy_234.filters = $.wbcr_factory_clearfy_234.filters || $.wfactory_444.filters;
	//todo: Переопредление для совместимости со старыми версиями плагинов.
	$.wbcr_factory_clearfy_234.hooks = $.wbcr_factory_clearfy_234.hooks || $.wfactory_444.hooks;

	$.wbcr_factory_clearfy_234.app = $.wbcr_factory_clearfy_234.app || {
		/**
		...
		showNotice: function(message, type) {
			...
			$([document.documentElement, document.body]).animate({
				 ...
				 * Пример регистрации хука $.wfactory_444.hooks.add('wbcr/factory_clearfy_234/updated',
				 * function(noticeId) {});
				 * @param {string} noticeId - id уведомления
				 */
				$.wfactory_444.hooks.run('wbcr/factory_clearfy_234/showed_notice', [noticeId]);
		...
		hideNotice: function(noticeId) {
			...
			el.fadeOut(500, function(e) {
				 ...
				 * @param {string} noticeId - id уведомления
				 */
				$.wfactory_444.hooks.run('wbcr/factory_clearfy_234/hidded_notice', [noticeId]);
	...
	$.wfactory_444.hooks.add('core/components/activation_error', function(plugin, button, response) {
		// Выполняем код ниже, только на страницах плагина с интерфейсом Clearfy
		if( !($('#WBCR').length && $.wbcr_factory_clearfy_234) ) {
			return false;
		}
		...
		if( response.data && response.data.error_message ) {
			$.wbcr_factory_clearfy_234.app.showNotice(response.data.error_message, 'danger');
		}
	});
	...
	$.wfactory_444.hooks.add('core/components/ajax_error', function(xhr, ajaxOptions, thrownError) {
		...
		$.wbcr_factory_clearfy_234.app.showNotice('Error: [' + thrownError + '] Status: [' + xhr.status + '] Error massage: [' + xhr.responseText + ']', 'danger');
	});
